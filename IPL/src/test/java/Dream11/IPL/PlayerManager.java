...
public class PlayerManager extends BaseClass
{
	...
	{	
	 ...
	 for(int i=1;i<TotalRow;i++)
	 {
		 ...
		 //String PlayerTeamName= Y1.getRow(i).getCell((short) 4).getStringCellValue();
	...
	public static Map<String, ArrayList<Object>> getAllPlayersOfSpecificType(String PlayerType) throws IOException
	{
		...
		for(Entry<String, ArrayList<Object>> entry : m1.entrySet())
		{
			...
			{
				//log.info("Player Name: "+entry.getKey()+",[ Details: Team-> "+entry.getValue().get(3)+  ", Type-> "+entry.getValue().get(0)+ ", Points-> "+entry.getValue().get(1)+ ", Credits-> "+entry.getValue().get(2)+" ]");
				newMap.put(entry.getKey(), entry.getValue());
			}
	...
	public static Map<String, ArrayList<Object>> GetTopPlayersOfSpecificType(Map<String, ArrayList<Object>>Team1,String PlayerType, int NoOfPlayers) throws IOException
	{
		int iCnt=0;
	...
	public static Map<String, ArrayList<Object>> GetTopPlayersOfSpecificType(String PlayerType, int NoOfPlayers) throws IOException
	{
		int iCnt=0;
	...
	public static ArrayList<ArrayList<Integer>> GetPossibleCombination(int sum)
{
	...
	static Map<String, ArrayList<Object>> sort(Map<String, ArrayList<Object>> unsortMap)
	{
        ...
        List<Map.Entry<String, ArrayList<Object>>> list = new LinkedList<Map.Entry<String, ArrayList<Object>>>(unsortMap.entrySet());
	...
	public static Map<String, ArrayList<Object>> updateCaptainViceCaptainPoints(Map<String, ArrayList<Object>> Team)
	{
		...
		for(Entry<String, ArrayList<Object>> entry : Dream11.entrySet())
		{
			if(icount==0 && icount<2)
			{
				...
				System.out.println();	;
				log.info("Captain: "+entry.getKey()+",[ Details: Team-> "+entry.getValue().get(3)+  ", Type-> "+entry.getValue().get(0)+ ", Points-> "+entry.getValue().get(1)+ ", Credit- "+entry.getValue().get(2)+" ]");
			}
			else if(icount==1 && icount<2)
			{
				entry.setValue(PlayerManager.updatePoint(entry.getValue(),1.5));
				System.out.println();	;
				log.info("Vice-Captain: "+entry.getKey()+",[ Details: Team-> "+entry.getValue().get(3)+  ", Type-> "+entry.getValue().get(0)+ ", Points-> "+entry.getValue().get(1)+ ", Credit- "+entry.getValue().get(2)+" ]");
			}
			icount++;
		}
	...
	public static boolean validatePlayerCap(Map<String, ArrayList<Object>> TeamBest4, Map<String, ArrayList<Object>> TeamNew7,String CombinationName)
	{
		...
		//log.info("Team1 Players vs Team2 Players in "+CombinationName+" : "+Team1Players+","+Team2Players);
...
//Finalised Dream11:
	...
	public static void MailMyDream11(String Team1,String Team2) throws MessagingException, IOException
	{
		...
		email.append("<p style=\"color:Tomato;\"><em><br/><b><i> Your Dream11 for "+Team1+" vs "+Team2+" is all set..</i></b><br/><em></p>");

		email.append("<p><b> Total Credits Used: </b>" +totalCredit +"<br/>"+
				 "<b>Total Points Gathered: </b>"+totalPoints+"</p>");
